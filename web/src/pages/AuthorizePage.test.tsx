...
test("completes desktop sign-in automatically and never renders returned credentials", async () => {
  ...
  const fetchMock = vi
    ...
    .mockImplementation(async (input) => {
      ...
      return new Response(
        JSON.stringify({
          redirect_uri:
            "http://127.0.0.1:43123/agentera/oauth/callback?code=opaque&state=opaque",
        }),
        { status: 200, headers: { "Content-Type": "application/json" } },
      );
    });
