...
export function DeleteAccountPage() {
  ...
  if (!config.identity_verification_available) {
    return <VerificationUnavailable />;
  }
  const recoveryMode =
    new URLSearchParams(location.search).get("mode") === "recover";
...
function VerificationUnavailable() {
  ...
  return (
    <PageFrame compact>
      <Card className="auth-card">
        <h1>{t("verificationUnavailableTitle")}</h1>
    ...
    <PageFrame compact>
      <Card className="auth-card wide-card">
        ...
        {status === "complete" ? (
          ...
          <form className="form-stack" onSubmit={recover}>
            <label>
...
function DeletionForm() {
  const { t } = useI18n();
  const [ownedWorkspaceCount, setOwnedWorkspaceCount] = useState<number | null>(
    null,
  );
  ...
  const remove = async (event: FormEvent) => {
    ...
    setError("");
    try {
      await requestDeletion(password, receipt);
      clearCSRFToken();
      setStatus("complete");
    } catch {
      setError(t("serviceError"));
  ...
  return (
    <PageFrame compact>
      <Card className="auth-card wide-card danger-card">
        <div className="danger-symbol">!</div>
        <h1>{t("deleteTitle")}</h1>
        <StatusMessage tone="warning">{t("deleteWarning")}</StatusMessage>
        {ownedWorkspaceCount !== null && (
          <StatusMessage tone="warning">{`${t("ownedWorkspaceDeletionPrefix")}${ownedWorkspaceCount}${t("ownedWorkspaceDeletionSuffix")}`}</StatusMessage>
        )}
        {(profileLoadFailed || error) && (
          <StatusMessage tone="error">
            {profileLoadFailed ? t("serviceError") : error}
          </StatusMessage>
        )}
        {status === "complete" ? (
          <>
            ...
            <Link className="primary-link" href="/delete-account?mode=recover">
              {t("recoverDeletion")}
            </Link>
          </>
        ) : (
        ...
        <div className="secondary-link">
          <Link href="/account">{t("accountOverview")}</Link>
        </div>
      </Card>
    </PageFrame>
  );
