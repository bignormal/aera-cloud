...
func NewInvitationSecret(entropy io.Reader) (InvitationSecret, error) {
	...
	return InvitationSecret{RawToken: rawToken, Digest: sha256.Sum256([]byte(rawToken))}, nil
}
...
func (InvitationSecret) GoString() string {
	return "organization.InvitationSecret{RawToken:REDACTED}"
}

func (InvitationSecret) Format(state fmt.State, _ rune) {
	_, _ = io.WriteString(state, "organization.InvitationSecret{RawToken:REDACTED}")
}

func (InvitationSecret) LogValue() slog.Value {
	return slog.StringValue("organization.InvitationSecret{RawToken:REDACTED}")
}
